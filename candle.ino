...
#define NUMBER_OF_FLAMES 2 // depends on number of neopixel triplets. 5 for 16 NeoPixel ring. 4 for 12 NeoPixel ring
...
// setup flame sensor
int flamePin = 7; // define the flame sensor interface
int flameVal = 0 ;// define numeric variables val
...
void setup()
{
  ...
  pinMode (soundPin, INPUT); // output interface defines the sound sensor
...
void killFlames() {
  Serial.println("Killing flames...");
  showFlames = false;
}

void doFlames() {
  Serial.println("Light flames...");
  showFlames = true;
}
...
void loop() {
  ...
  if (showFlames) {
    ...
    UpdateFlameColor(0, 0);
    UpdateFlameColor(1, 0);
    strip.show();    
  }
  delay(22);
}
